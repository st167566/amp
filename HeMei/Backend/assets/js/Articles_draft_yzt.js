...
var nv1 = new Vue({
    ...
    created: function () {
        ...
        $.ajax({
            type: "get",
            url: "Articles_draft_WebService.asmx/InitArticleMan",
        ...
        $.ajax({
            type: "get",
            url: "Articles_draft_WebService.asmx/InitCat",
        ...
        $.ajax({
            type: "get",
            url: "Articles_draft_WebService.asmx/InitAuthor",
            success: function (str) {
        ...
        $.ajax({
            type: "get",
            url: "Articles_draft_WebService.asmx/InitUserTag",
            success: function (str) {
                var a = $(str).find("string").text();
                nv1.usertag = eval('(' + a + ')');
            }
                ...
                for (var i in nv1.catitems) {
                    if (nv1.catitems[i].ID == nv1.catselect) {
                        if (nv1.catitems[i].Subs == 0) nv1.has = false;
                            ...
                            $.ajax({
                                type: "get",
                                url: "Articles_draft_WebService.asmx/InitSub",
...
function updateItem() {
    ...
    $.ajax({
        type: "get",
        url: "Articles_draft_WebService.asmx/UpdateArticleMan",
        async: false,  //同步，既做完才往下
...
$('#layer').on('show.bs.modal', function (e) {
    ...
    if (ids != "") {
        $.ajax({
            type: "get",
            url: "Articles_draft_WebService.asmx/SelectArticle",
            async: false,  //同步，既做完才往下
            data: { "ids": ids },
            success: function (str) {
                var data = $(str).find("string").text();
...
function deleteArticles() {
    ...
    if (articleids != "") {
        $.ajax({
            type: "get",
            url: "Articles_draft_WebService.asmx/DeleteArticle",
            data: { "ids": articleids },
            success: function (str) {
                alert("已成功移至回收站");
...
function topArticle() {
    ...
    if (articleids != "") {
        $.ajax({
            type: "get",
            url: "Articles_draft_WebService.asmx/TopArticle",
            data: { "ids": articleids },
            success: function (str) {
                alert("已成功置顶");
                window.location.reload();
...
function moveArticles() {
    ...
    if (articleids != "") {
        $.ajax({
            type: "get",
            url: "Articles_draft_WebService.asmx/MoveArticle",
            data: {
                "ids": articleids,
                "catid": $('#move_cat').find("option:selected").val(),
                "catname": $('#move_cat').find("option:selected").text(),
                "subid": nv1.has1 ? $('#move_sub').find("option:selected").val() : "",
                "subname": nv1.has1 ? $('#move_sub').find("option:selected").text() : "",
            },
            success: function (str) {
                alert("已成功移动文章");
                window.location.reload();
